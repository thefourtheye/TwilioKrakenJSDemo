...
module.exports = function(options) {
    ...
    return {
        ...
        "fetch": function(database, table, columns, criteria, exact, sort, callBack) {
            var tableName = database && _.isString(database) ? database + "." : "" + table,
                query = "SELECT " +
                (_.isArray(columns) ? columns.join(", ") : columns) + " FROM " + tableName,
                queryValues = [], queryColumns = [], statement;

            if (_.isPlainObject(criteria)) {
                queryValues = _.values(criteria);
                queryColumns = _.keys(criteria);
            }
            ...
            if (_.isPlainObject(sort)) {
                query += " ORDER BY " + _.keys(sort).map(function(currentColumn) {
                    return currentColumn + " " + sort[currentColumn];
                }).join(", ");
            }
            logger("DEBUG", query);
